...
# Within initialize method, split word into array1 [parameter: string, return value: array]
...
# Define a compare method [parameter: string, return value: array] that takes user2's letter and iterates over array1 to find index value of matches. Inserts those values into array2 at matching indices.
# Create a loop so user2 can enter guesses until guesscount is 0 or there are no instances of "-" in Array2
...
class Guessword
  attr_reader :guessword_array, :guessed_array
    ...
    until guess_counter == 0 || !game_word.guessed_array.include?("-")
      letter = gets.chomp
        if user_guesses.include?(letter)
          ...
          user_guesses << letter
          guess_counter -= 1
            if game_word.guessed_array.include?("-")
              puts "#{guess_counter} guesses remaining!"
            end
        end
    end
